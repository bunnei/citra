...
{
#if defined(_M_X86) || defined(_M_ARM_64)
    u64 h[4] = { len, 0, 0, 0 };
    u32 Step = (len / 8);
...
static u64(*HashFunction)(const u8 *src, u32 len, u32 samples) = &GetMurmurHash3;

u64 GetHash64(const u8 *src, u32 len, u32 samples)
{
    return HashFunction(src, len, samples);
}
