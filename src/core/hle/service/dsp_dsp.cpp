...
void SignalInterrupt() {
    ...
    // application that a DSP interrupt occurred, without specifying which one. Since we don't
    ...
    // DSP interrupts, and trigger them at more appropriate times.

    if (interrupt_event == 0) {
        LOG_WARNING(Service_GSP, "cannot synchronize until DSP event has been created!");
        return;
    }
